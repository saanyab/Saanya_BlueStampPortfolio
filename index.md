...
I completed my first milestone by successfully setting up and connecting an ultrasonic sensor and an Arduino microcontroller to a breadboard. This allowed me to test and make sure the LED, vibrating sensor, and buzzer were working correctly. I also figured out how to connect everything properly. I used jumper cables to create a link between the Arduino and the ultrasonic sensor. The sensor had a switch that let me choose between using the buzzer or the LED in my project. Then, I wrote a program that displayed the distance of an object in centimeters on the computer screen through the serial monitor. I achieved this by using the ultrasonic sensor. When an object comes closer to the sensor, the distance reading gets smaller, and as the object moves farther away, the reading gets larger. The sensor works by sending out sound waves that travel towards the object. When the sound waves bounce back to the sensor, it detects the echo. By measuring the time it takes for the echo to return, the sensor can accurately calculate the distance between itself and the object.
